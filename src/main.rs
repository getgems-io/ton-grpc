...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let tw = stream::iter((info.last.seqno - 1000)..info.last.seqno)
        .then(|seqno| {
            let client = &client;

            async move { get_tx_from_seqno(client, seqno, info.last.seqno) }
        })
...
async fn get_tx_from_seqno(
    client: &AsyncClient,
    seqno: u64,
    max: u64,
) -> anyhow::Result<Vec<ShortTxId>> {
    let shards = client.get_shards(seqno, 0).await?;
    let txs = stream::iter(shards).flat_map(|shard| client.get_tx_stream(shard));
