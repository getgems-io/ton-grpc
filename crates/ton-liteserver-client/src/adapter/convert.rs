...
pub fn shard_descr_to_block_id_ext(workchain_id: i32, shard: &ShardDescr) -> BlockIdExt {
    BlockIdExt {
        ...
        root_hash: base64_standard.encode(shard.root_hash),
        file_hash: base64_standard.encode(shard.file_hash),
    }
}
...
pub fn block_header_to_ton_client(
    ...
    ton_tower::response::BlockHeader {
        ...
        // TODO: after full migration to ADNL, return info.prev_key_block_seqno unconditionally
        prev_key_block_seqno: if !info.not_master {
            info.prev_key_block_seqno as i32
        } else {
            0
        },
...
fn tlb_message_to_ton_client(cell: &Arc<Cell>) -> anyhow::Result<ton_tower::response::Message> {
    let msg: Message = cell
        .parse_fully(())
        .map_err(|e| anyhow!("failed to parse message: {e}"))?;

    let hash = base64_standard.encode(cell.hash());

    let body_cell = msg.body.to_cell(()).map_err(|e| anyhow!("{e}"))?;
    let body_hash = base64_standard.encode(body_cell.hash());

    let body = {
        let boc = BoC::from_root(Arc::new(body_cell));
        let bytes = boc
...
pub fn transaction_to_ton_client(
    ...
    let out_msgs: Vec<ton_tower::response::Message> = out_msgs
        .into_iter()
        .map(|(_, cell)| tlb_message_to_ton_client(cell))
