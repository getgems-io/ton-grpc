...
use crate::tl::LiteServerSendMessage;
use anyhow::anyhow;
use base64::Engine;
...
const SEND_MSG_STATUS_OK: i32 = 1;

#[async_trait::async_trait]
impl ton_client::MessageClient for LiteServerClient {
    async fn send_message(&self, message: &str) -> anyhow::Result<()> {
        let body = decode_message_body(message)?;

        let response = self
            .clone()
            .oneshot(LiteServerSendMessage { body })
            .await
            .map_err(|e| anyhow!(e))?;

        if response.status != SEND_MSG_STATUS_OK {
            return Err(anyhow!(
...
fn decode_message_body(message: &str) -> anyhow::Result<Vec<u8>> {
    base64_standard
        .decode(message)
...
fn compute_message_hash(body: &[u8]) -> anyhow::Result<String> {
...
mod integration {
    ...
    async fn send_message_rejects_invalid_base64() -> anyhow::Result<()> {
        ...
        let result = client.send_message("not_base64!@#$").await;
    ...
    async fn send_message_rejects_invalid_boc_at_validator() -> anyhow::Result<()> {
        let (client, _server) = setup().await?;
        let garbage_boc = base64_standard.encode([0xde, 0xad, 0xbe, 0xef]);

        let result = client.send_message(&garbage_boc).await;
